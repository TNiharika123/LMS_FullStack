...
const ScheduleLecture = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      // Check file size - limit to 1MB instead of 2MB to prevent payload issues
      if (file.size > 1 * 1024 * 1024) { 
        setError("Image file is too large. Please select an image smaller than 1MB.");
        return;
      }
      
      // Create a smaller preview using canvas
      ...
      reader.onloadend = () => {
        ...
        img.onload = () => {
          ...
          // Get the resized image as base64 with reduced quality for smaller size
          const resizedImage = canvas.toDataURL('image/jpeg', 0.6);
          
          setImagePreview(resizedImage);
          setLecture(prev => ({ ...prev, image: resizedImage }));
        };
              ...
              <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                ...
                {/* <Box sx={{ mt: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                    ...
                    <Typography variant="subtitle2">Lecture Thumbnail Image (Optional)</Typography>
                  </Box> */}
                  ...
                  <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
                    Recommended size: 800x450px (16:9 ratio). Max size: 1MB
                  </Typography>
                </Box> */}
